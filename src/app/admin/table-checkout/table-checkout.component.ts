...
export class TableCheckoutCardComponent implements OnInit ,OnChanges {
  ...
  fetchOrders(tableId: string) {
    // Implement your logic to fetch orders based on the selected tableId
    // For example, call a service method to get orders from the server
    // this.orders = this.orderService.getOrdersForTable(tableId);
    console.log(`Fetching orders for table: ${tableId}`);
     this.api.getOrderoftables(this.resid,tableId).subscribe(
      (res: any[]) => {
        console.log('table from table-checkoutorder:', res);
